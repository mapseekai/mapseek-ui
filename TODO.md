...
- [ ] **53. 静默截断无发现路径 ×7 处**：✅ LayerPanel 替换后，`LayerGroup` 的分组名、图层名和元数据均提供原生 `title`；✅ Toolbox 的工具名与说明均提供原生 `title`；✅ ResourceSidebar 的分类名已提供原生 `title`；仍待处理：`registry/blocks/resource-detail-drawer/ResourceDetailDrawer.tsx:44`（KV 值 `max-w-[180px] truncate`，路径/URL/id 无 title/Tooltip）、`registry/blocks/notification-center/NotificationCenter.tsx:228,231,234`（title/description/sourceUid）、`registry/blocks/linked-ref-list/LinkedRefList.tsx:70,141`、`registry/blocks/style-editor-panel/StyleEditorPanel.tsx:35,75,78`、`registry/blocks/layer-style-editor/LayerStyleEditor.tsx:48`、`registry/blocks/app-top-bar/AppTopBar.tsx:55`。契约："Long identifiers and data values must truncate with a discoverable full value or scroll horizontally"（MapSwitcher/StyleSourcePickerDialog 有原生 title 合规；attr-table overflow tooltip 合规）。
...
- [x] **57. alert 默认变体使用保留绿**：✅ 已修复：默认 Alert 改用 `border-info/30 bg-info/10 text-info`，被动消息采用 `role="status"`，破坏性消息继续采用 `role="alert"`；不再占用 primary 动作绿。
