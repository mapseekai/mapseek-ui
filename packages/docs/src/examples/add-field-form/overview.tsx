...
export type AddFieldFormDemoLabels = {
  readonly intro: string
  readonly reset: string
  readonly valid: string
  readonly invalid: string
  readonly form: AddFieldFormLabels
}

export const zhAddFieldFormLabels = {
...
} satisfies AddFieldFormDemoLabels

export const enAddFieldFormLabels = {
  intro:
...
export function AddFieldFormDemo({ labels }: { readonly labels: AddFieldFormDemoLabels }) {
  const [value, setValue] = useState<AddFieldValue>({ ...EMPTY_ADD_FIELD, type: "text" })
  const isValid = value.name.trim().length > 0

  return (
    <div data-demo="add-field-form" className="flex w-full flex-col gap-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[420px_minmax(0,1fr)]">
        ...
        <div className="flex min-w-0 flex-col gap-2">
          ...
          >
            {isValid ? labels.valid : labels.invalid}
          </span>
