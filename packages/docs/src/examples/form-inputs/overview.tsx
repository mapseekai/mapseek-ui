...
export type FormInputsDemoLabels = {
  readonly intro: string
  readonly reset: string
}

export const zhFormInputsLabels = {
  intro: "commit-on-blur 输入件集合；短枚举使用内联选项，长枚举使用下拉或自动补全。",
  reset: "重置值",
} satisfies FormInputsDemoLabels

export const enFormInputsLabels = {
  intro:
    "Commit-on-blur inputs; short enums use inline choices and longer sets use select/autocomplete.",
  reset: "Reset values",
} satisfies FormInputsDemoLabels

export function FormInputsDemo({ labels }: { readonly labels: FormInputsDemoLabels }) {
  ...
  return (
    <div data-demo="form-inputs" className="flex w-full max-w-[520px] flex-col gap-4">
