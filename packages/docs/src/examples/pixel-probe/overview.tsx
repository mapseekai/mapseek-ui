...
export const zhPixelProbeLabels = {
  reopen: "重新打开",
  copied: "已复制 JSON",
  closed: "已关闭",
  empty: "暂无选中像元",
...
} satisfies PixelProbeDemoLabels

export const enPixelProbeLabels = {
  reopen: "Reopen",
  copied: "Copied JSON",
  closed: "Closed",
...
export function PixelProbeDemo({ labels }: { readonly labels: PixelProbeDemoLabels }) {
  const [point, setPoint] = useState(1)
  const [closed, setClosed] = useState(false)
  const [empty, setEmpty] = useState(false)
  const [status, setStatus] = useState(`${labels.statusPrefix} 1`)

  if (closed) {
    return (
      <div data-demo="pixel-probe" className="flex flex-col gap-2">
        <button
          type="button"
          ...
          onClick={() => {
            setClosed(false)
            setEmpty(false)
            setStatus(`${labels.statusPrefix} ${point}`)
          }}
        >
          {labels.reopen}
        </button>
          ...
          <PixelProbe
            ...
            onPrev={() =>
              setPoint((current) => {
                ...
                setStatus(`${labels.statusPrefix} ${nextPoint}`)
                return nextPoint
              })
            }
            onNext={() =>
