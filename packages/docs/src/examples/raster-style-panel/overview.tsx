...
export const enRasterStylePanelLabels = {
...
export function RasterStylePanelDemo({
  ...
  readonly labels: typeof zhRasterStylePanelLabels
}) {
  const [value, setValue] = useState<RasterStyleValue>(initialStyle)
  const [valid, setValid] = useState(true)
  const [revision, setRevision] = useState(0)
  const [status, setStatus] = useState(labels.valid)

  return (
    <section data-demo="raster-style-panel" className="grid gap-3 lg:grid-cols-[380px_1fr]">
      <div className="border border-border bg-card">
        <div className="flex items-center justify-between border-b border-border px-3 py-2">
          <h3 className="m-0 text-sm font-semibold">{labels.title}</h3>
          <span
      ...
      <div className="flex min-w-0 flex-col gap-3 border border-border bg-muted/35 p-3">
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            disabled={!valid}
            onClick={() => setStatus(labels.saved)}
          >
            {labels.save}
          </Button>
        </div>
        ...
        <div className="min-w-0 border border-border bg-background p-2">
          <p className="m-0 mb-2 text-xs font-medium text-muted-foreground">{labels.summary}</p>
