...
export function StorageMeterDemo({ labels }: { readonly labels: typeof zhStorageMeterLabels }) {
  ...
  return (
    <div data-demo="storage-meter" className="flex flex-col gap-4">
      <div className="flex flex-wrap gap-2">
        <Button
          ...
          onClick={() => selectMode("normal", labels.normal)}
        >
          {labels.normal}
        </Button>
        <Button
          ...
          onClick={() => selectMode("full", labels.full)}
        >
          {labels.full}
        </Button>
        <Button
          ...
          onClick={() => selectMode("unsupported", labels.unsupportedMode)}
        >
          {labels.unsupportedMode}
        </Button>
        <span
      ...
      <div className="flex h-[260px] items-start justify-end border border-border bg-muted/20 p-3">
        <StorageMeter
          ...
          labels={labels}
          error={error}
          errorLabel={() => labels.errorText}
          onRefresh={() => setStatus(labels.refreshed)}
          footer={
