...
export function StyleEditorPanelDemo({
  labels,
}: {
  ...
  function removeSource(id: string, title: string) {
    ...
    setStatus(`${labels.removed}: ${title}`)
  }

  return (
    <section data-demo="style-editor-panel" className="space-y-3">
      <div className="h-[520px] max-w-[420px] border border-border">
        <StyleEditorPanelRoot dataWdKey="docs:style-editor-panel">
          ...
          />
          <StyleEditorPanelContent>
            <StyleEditorPanelSection title={labels.active}>
              {sources.map((source) => (
                <StyleEditorPanelCard
                  key={source.id}
                  title={source.title}
                  ...
                  meta={
                    <Badge variant="outline">
                      {source.tone === "tileset" ? labels.tileset : labels.dataset}
                    </Badge>
                  }
                  action={
                    <Button
                      ...
                      size="icon-xs"
                      variant="destructive"
                      aria-label={`${labels.remove}: ${source.title}`}
            ...
            <StyleEditorPanelSection title={labels.emptySection}>
              <StyleEditorPanelEmpty>{labels.empty}</StyleEditorPanelEmpty>
            </StyleEditorPanelSection>
          </StyleEditorPanelContent>
        </StyleEditorPanelRoot>
      </div>
