...
it("includes every public docs route in autogenerated sidebar navigation", async () => {
  const componentPage = await readFile("packages/docs/build/components/button/index.html", "utf8")
  const blockPage = await readFile("packages/docs/build/blocks/layer-panel/index.html", "utf8")
...
it("keeps displayed example source as exact TSX source", async () => {
  ...
  expect(js).toContain("export function ButtonBasicDemo({ labels }")
