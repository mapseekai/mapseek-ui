...
export function assertCompleteInventory(items: readonly RegistryItem[]): void {
  ...
  const missing = [...expectedTypeByName]
  ...
  const wrongType = [...expectedTypeByName]
    ...
    .map(([name]) => name)
  if (missing.length || unexpected.length || wrongType.length)
    throw new Error(
      `inventory mismatch: missing ${missing.join(", ")}; unexpected ${unexpected.join(", ")}; wrong type ${wrongType.join(", ")}`,
    )
}
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  ...
  if (process.argv.includes("--complete")) assertCompleteInventory(items)
}
