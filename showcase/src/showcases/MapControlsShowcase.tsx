...
export function MapControlsDemo({ locale = "zh-CN" }: LocalizedDemoProps) {
  const demoLabels = labels[locale]
  const [status, setStatus] = useState(demoLabels.idle)
    ...
    <div className="flex w-full flex-col gap-2">
      <div className="relative h-[320px] overflow-hidden border border-border bg-muted/20">
        ...
        <MapControls
          ...
          labels={demoLabels.controls}
          onZoomIn={() => setStatus(demoLabels.actions.zoomIn)}
          onZoomOut={() => setStatus(demoLabels.actions.zoomOut)}
          onLocate={() => setStatus(demoLabels.actions.locate)}
          onHome={() => setStatus(demoLabels.actions.home)}
        />
      </div>
