...
export function ToggleConfigPopoverDemo({ locale = "zh-CN" }: LocalizedDemoProps) {
    ...
    <section className="space-y-3">
      <div className="inline-flex h-10 items-center border border-border bg-card px-1 shadow-[var(--shadow-map-float)]">
        <ToggleConfigPopover
          ...
          toggleLabel={demoLabels.toggleLabel}
          triggerLabel={demoLabels.triggerLabel}
          tooltip={`${demoLabels.toggleLabel}: ${threshold}px`}
          switchLabel={demoLabels.switchLabel}
          checked={enabled}
          onCheckedChange={setEnabled}
          labels={{ settingsSuffix: demoLabels.settingsSuffix }}
        >
          <div className="border-b border-border px-3 py-2.5">
            <div className="mb-2 flex items-center">
              <span className="flex-1 text-[11px] leading-[14px] font-medium tracking-[0.06em] text-muted-foreground uppercase">
                {demoLabels.threshold}
              </span>
            ...
            {toggleTargets.map(({ key, Icon }) => (
              ...
              >
                ...
                <span className="text-xs leading-none font-medium text-foreground">
                  {key === "vertex" ? demoLabels.vertex : demoLabels.edge}
                </span>
              </label>
            ))}
          </div>
      ...
      <p data-demo-status="toggle-config-popover" className="m-0 font-mono text-xs">
        {enabled ? demoLabels.enabled : demoLabels.disabled} / {threshold}px
      </p>
    </section>
  )
}
